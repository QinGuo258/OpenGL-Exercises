...
| 类别 | 特性 |
|------|------|
| **光照与阴影** | 动态昼夜循环 + 月光阴影、4096² Poisson Disk 16 采样软阴影、512 个动态点光源 + 手持火把、动态阴影正交范围 (80×80m) |
| **后处理管线** | HDR (RGBA16F) → Reinhard → ACES 电影级色调映射 → Gamma 2.4、双通道高斯 Bloom、SSAO 三阶段环境光遮蔽（植被 SSAO 减弱防止伪影）、60 步屏幕空间 God Rays + 太阳距离遮罩、**屏幕空间反射 (SSR)** |
| **水体与特效** | 顶点波浪动画 + 有限差分法线扰动 + Fresnel 反射 + 太阳耀斑、植被风摆（2 种模式，与 G-Buffer 同步消除 SSAO 鬼影）、10000 雨滴 VBO-less GPU 粒子 + 深度剔除防漏雨、**4 变体随机雨声**循环播放 |
| **程序化天空** | 体积云 (16+2 步 Marching)、网格化哈希星空 (Anti-Flicker)、Minecraft 风格方块日月、晨昏渐变 |
| **骨骼动画** | 双轨混合 (Base + Overlay)、头部追踪、第一人称头部隐藏 + 骨骼矩阵重算修复头部阴影、绑定姿态注入修复、无限循环 |
| **角色物理** | 手写运动学控制器 (6 阶段)、动态 FOV 冲刺效果 (45°→55° 平滑过渡)、Möller–Trumbore 射线碰撞、2D 空间哈希加速、防隧道效应 |
| **战斗系统** | 精准准星射线判定 (Slab AABB)、横扫之刃 AOE、**潜行攻击**（潜行状态下伤害提升）、箭矢抛物弹道、受击闪红 + 击退 |
| **双类型 AI** | 僵尸 (追击 + 撕咬)、骷髅 (远程射击 + 后退风筝)、双轨动画混合 |
| **中文 UI** | stb_truetype 烘焙 21,000+ 汉字 (8192² GL_RED 图集)、像素级对齐 HUD、F8 一键隐藏 HUD、UTF-8 解码 |
| **故事系统** | 6 阶段脚本化线性叙事 (纯代码驱动)、电影级过场、对话/消息系统 |
| **3D 音效** | miniaudio 引擎、MP3 解码 + 空间定位、10+ 音效文件（含 4 变体随机雨声循环） |
| **Shader 热重载与调参** | F1 全 Shader 热重载 + `tuning.json` 运行时参数配置（编译失败旧程序保留），40+ 可调参数无需重编译 |
...
| **F6** | 碰撞调试线框 |
| **F8** | 切换 HUD 显示（快捷栏/心形/准星/第一人称手臂） |
| **R** | 切换降雨（2s 平滑过渡，4 变体随机雨声） |
| **T** | 聊天历史面板 |
| **Esc** | 退出程序 |
