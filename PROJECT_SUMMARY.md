...
本项目是一个**使用现代 C++20 和 OpenGL 4.6 Core Profile 从零实现的 3D 渲染框架**。项目的核心目标是深入理解实时渲染管线的底层工作原理——每一行着色器代码、每一个矩阵运算、每一段物理逻辑均为手写实现，不依赖 Unity、Unreal Engine 或其他任何现成图形中间件。
