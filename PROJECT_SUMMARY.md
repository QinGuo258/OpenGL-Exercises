...
本项目是一个**使用现代 C++20 和 OpenGL 4.6 Core Profile 从零实现的 3D 渲染框架**。项目的核心目标是深入理解实时渲染管线的底层工作原理，不依赖 Unity、Unreal Engine 或其他任何现成图形中间件。
...
- **屏幕空间反射 (SSR)**：半分辨率光线步进 + 天空回落 + 按材质菲涅尔加权。
...
按下 **F1** 键，着色器从源目录重新读取、重新编译、原子交换 GL Program。编译失败时保留旧程序，不会引发运行时崩溃。这种**零重启-所见即所得**的调试流程极大地加速了着色器开发迭代——传统图形开发中每次修改着色器需重启整个应用程序，而热重载将这一过程缩短为亚秒级，是本项目开发效率的关键支撑。
...
> **一切高级特效均为纯 Shader 数学实现。本项目未使用任何第三方图形引擎、特效中间件或成品渲染框架。**
...
2. 60 次径向采样 + 深度距离遮罩：从太阳屏幕位置向边缘沿径向步进，指数衰减 (`uDecay = 0.92`)，辅以距离深度遮罩过滤被遮挡的非太阳像素（如建筑物后方不产生 God Rays）。
...
半分辨率 (960×540) 后处理 Pass — 在 HDR 场景组装完成后、Bloom 亮部提取之前执行：

1. **视图空间光线步进**：40 步线性步进 + 4 步二分精炼，沿反射方向在屏幕空间中查找命中几何体。
2. **穿越检测**：检测光线深度从"表面前方"翻转到"表面后方"的时刻，100% 捕获命中。
3. **天空回落**：光线射向天空未命中时，从 HDR 场景中采样真实天空颜色（含云层和太阳光晕）。
4. **按材质类型混合**：水面 (Fresnel 0.02→0.98) 反射最强，手持物品 Schlick 菲涅尔反射，地面/植物/自发光体不反射。
5. **上一帧重投影**：当前帧 SSR 复制到 `ssrColorTexPrev`，下一帧 `model.frag` 采样 — 解决 SSR 需要已完成场景颜色的"鸡生蛋"问题。
...
- **菲涅尔反射**：`F = mix(0.04, 1.0, pow(1 - V·N, 5))`——掠射角反射增强、正视透明，反射源优先使用 SSR 屏幕空间反射，未命中时回退至天空渐变颜色 `uHorizonColor`/`uZenithColor`。
...
- 此修复避免了手动在 Blockbench 中补充关键帧的繁琐流程，确保模型动画在渲染器中的表现与 Blockbench 预览一致。
...
**所有着色器**（vertex + fragment，共 26 个文件）通过 `allShaders` 向量支持**运行中无重启重新编译**。按下 F1 遍历向量逐项调用 `Shader::Reload()` → 从 `SHADER_SRC_DIR` 编译时注入的绝对路径重新读取源文件 → 编译 → 原子替换 GL Program。编译失败时旧程序保留不变，错误信息输出到 stderr。这种调试流程将着色器开发从"编辑→重启→重新定位场景→验证"的分钟级循环缩短为**亚秒级所见即所得**——是项目着色器开发效率的核心保障。
...
**工程实践领悟**：在一个人独立完成从渲染器、着色器、物理、UI、音效到游戏系统的完整项目中，最重要的能力不是某个特定领域的深度知识，而是**系统化的问题分解与逐层调试能力**——当屏幕显示不正确时，需要快速判断问题是出在模型加载、矩阵变换、着色器逻辑、FBO 配置还是深度测试设置，这需要建立一套完整的"渲染管线心智模型"。
